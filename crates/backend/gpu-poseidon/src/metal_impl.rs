...
use koala_bear::{
    KoalaBear, poseidon1_final_constants, poseidon1_initial_constants, poseidon1_sparse_first_round_constants,
...
fn poseidon1_lambda_over_16_for_metal() -> [KoalaBear; 16] {
    let mut lambda_br = KoalaBear::new_array([1, 3, 13, 22, 67, 2, 15, 63, 101, 1, 2, 17, 11, 1, 51, 1]);
...
#[inline(always)]
fn bt(f: &mut [KoalaBear; 16], lo: usize, hi: usize) {
    let a = f[lo];
    let b = f[hi];
    f[lo] = a + b;
    f[hi] = a - b;
...
#[inline(always)]
fn neg_dif(f: &mut [KoalaBear; 16], lo: usize, hi: usize, twiddle: KoalaBear) {
    let a = f[lo];
    let b = f[hi];
    f[lo] = a + b;
    f[hi] = (b - a) * twiddle;
...
#[inline(always)]
fn dif_ifft_16_mut(f: &mut [KoalaBear; 16]) {
    let w1 = KoalaBear::new(0x08dbd69c);
    let w2 = KoalaBear::new(0x6832fe4a);
    let w3 = KoalaBear::new(0x27ae21e2);
    let w4 = KoalaBear::new(0x7e010002);
    let w5 = KoalaBear::new(0x3a89a025);
    let w6 = KoalaBear::new(0x174e3650);
    let w7 = KoalaBear::new(0x27dfce22);
...
fn generate_msl_source() -> String {
    ...
    let lambda = poseidon1_lambda_over_16_for_metal();
    let lambda_arr: [u32; 16] = core::array::from_fn(|i| kb_to_monty_u32(lambda[i]));
    emit_u32_array(&mut s, "LAMBDA", lambda_arr);

    let mds_circ_col = KoalaBear::new_array([1, 3, 13, 22, 67, 2, 15, 63, 101, 1, 2, 17, 11, 1, 51, 1]);
    let mds_circ_col_arr: [u32; 16] = core::array::from_fn(|i| kb_to_monty_u32(mds_circ_col[i]));
    emit_u32_array(&mut s, "MDS_CIRC_COL", mds_circ_col_arr);
...
mod tests {
    ...
    fn monty_to_kb_arr<const N: usize>(arr: [u32; N]) -> [KoalaBear; N] {
        // SAFETY: KoalaBear is #[repr(transparent)] over u32 in Montgomery form.
    ...
    fn hash_leaves_parity_vs_cpu() {
        ...
        let initial_state_kb = precompute_zero_suffix_state::<KoalaBear, _, 16, 8, 8>(&p1, 2);
        ...
        for r in 0..height {
            ...
            cpu_digests.push(kb_arr_to_monty(state[..8].try_into().unwrap()));
        }
        ...
        for (i, (cpu, gpu)) in cpu_digests.iter().zip(gpu_digests.iter()).enumerate() {
            assert_eq!(monty_to_kb_arr(*cpu), monty_to_kb_arr(*gpu), "row {i} mismatch");
        }
    }
    ...
    fn no_initial_full_tree_parity_vs_cpu() {
        ...
        for r in 0..height {
            let stream: Vec<KoalaBear> = (0..n_pad)
                ...
                .collect();
            let digest: [KoalaBear; 8] = symetric::hash_rtl_iter::<KoalaBear, _, _, 16, 8, 8>(&p1, stream);
        ...
        for (i, (cpu, gpu)) in cpu_digests.iter().zip(gpu_layers[0].iter()).enumerate() {
            assert_eq!(monty_to_kb_arr(*cpu), monty_to_kb_arr(*gpu), "leaf row {i} mismatch");
        }

        let cpu_layers_kb: Vec<Vec<[KoalaBear; 8]>> = {
            let mut layers: Vec<Vec<[KoalaBear; 8]>> = Vec::new();
            layers.push(cpu_digests.iter().copied().map(monty_to_kb_arr).collect());
        ...
        for (k, (cpu_layer, gpu_layer)) in cpu_layers_kb.iter().zip(gpu_layers.iter()).enumerate() {
            ...
            for (i, (cpu, gpu)) in cpu_layer.iter().zip(gpu_layer.iter()).enumerate() {
                assert_eq!(*cpu, monty_to_kb_arr(*gpu), "layer {k} row {i} mismatch");
            }
        }
    }
}
