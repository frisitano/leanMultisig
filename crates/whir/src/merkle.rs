...
/// GPU path: build the entire Merkle tree, including leaf digests and all
/// parent layers, in one Metal command buffer. Returns `None` when Metal is not
/// enabled/available or when the tree shape is outside the kernel's supported
/// power-of-two reduction path.
...
fn try_gpu_build_merkle_tree(
    ...
    let layers_u32 =
        gpu_poseidon::build_merkle_tree_full(matrix_u32, height, leaf.width(), effective_base_width, &initial_u32);
...
/// GPU path for the `effective == full` case, where there is no precomputed
/// zero-suffix sponge state. This is used by WHIR rounds whose row stream starts
/// directly from the matrix row.
...
fn try_gpu_build_merkle_tree_no_initial(
    ...
    let height = leaf.height();
    if !height.is_power_of_two() || full_base_width < 16 || !full_base_width.is_multiple_of(8) {
        return None;
    }

    let matrix_u32: &[u32] =
        unsafe { std::slice::from_raw_parts(leaf.values.as_ptr() as *const u32, leaf.values.len()) };
    let layers_u32 = gpu_poseidon::build_merkle_tree_full_no_initial(matrix_u32, height, leaf.width(), full_base_width);
    let digest_layers: Vec<Vec<[KoalaBear; DIGEST_ELEMS]>> = unsafe { std::mem::transmute(layers_u32) };
    Some(symetric::merkle::MerkleTree { digest_layers })
}
